...
# replace sed with cut - spoiler - its slower!!!
...
while read -r GROUP ; do
  FIRST=$(echo $GROUP | head -n 1 | cut -f 1 -d ' ')
  ALL_SELECTED=$(echo $FIRST | sed 's/\(.\)/\1=0 /g')
  GROUP_SIZE=$(echo $GROUP | wc -w)

  for PERSON in $GROUP ; do
    for CHAR in $(echo "$PERSON" | grep -o '.'); do
      if $(echo "$ALL_SELECTED" | grep -q "$CHAR" ); then
        CUR_COUNT=$(echo $ALL_SELECTED | grep -o "$CHAR=\S*" | cut -f 2 -d '=')
